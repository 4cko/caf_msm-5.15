 ...
 * Ported for Linux 5.15 compatibility.
 */
...
int __init prefer_silver_init(void)
{
	...
	/* * 5.15 API Update: register_sysctl replaces the need to build a root
	 * table array, directly nesting `prefer_silver_table` under `/proc/sys/kernel/` 
	 */
	ps_sysctl_header = register_sysctl("kernel", prefer_silver_table);
	if (!ps_sysctl_header) {
